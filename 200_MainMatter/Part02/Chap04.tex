...
% 200_MainMatter/Part02/Chap04.tex -- Chapter 04

\ChapterGeometry

\chapter{RBC 模型：数值求解}\label{ch:04}
...
在\ref{ch:03}中，我们已经实现了 RBC 模型的对数线性化
...
\begin{table}[H]
    ...
    >{\raggedright\arraybackslash}X}
        ...
        & 参数 & 季度值 & \quad 备注 \\
        \midrule
        & \(\beta\) & 0.990 & \quad 季折现率 0.99 对应年利率约 4.1\% \sidenote{从欧拉方程 \sidemath{\(u'_t=\beta\,\meant\bks{u'\nt\bka{1+r\nt}}\)} 得 \sidemath{\(r=\bka{1-\beta}/\,\beta\)}}\\
...
\begin{figure}[H]
    ...
    \MaybeIncludeGraphics[width=\textwidth]{_asset/figures/ch04-IRF.png}
  \caption{技术冲击下各变量对数偏离的脉冲响应路径}
  \label{fig:IRFfig}
\end{figure}

\noxmubluebf{问题：为什么资本路径呈驼峰形？}

首先我们要明确：\(\hat k\lt\) 上升或下降等同于 \(K\lt\) 上升或下降，所以图像说明了 \(K_t\) 的路径呈现\,\xmured{驼峰形}（hump-shaped）。根据资本的运动方程
\begin{equation*}
...
\noxmubluebf{问题：为什么资本回报会小于稳态？}

\sidenote{\centering\MaybeIncludeGraphics[width=\linewidth]{_asset/figures/ch04-IRF-R.png}}
...
\begin{figure}[H]
    \centering
    \MaybeIncludeGraphics[width=0.7\textwidth]{_asset/figures/ch04-sigma.png}
...
\end{equation*}
从中整理出作差结果，再让两个式等零，找到\,\xmured{边界路径}（locus）
\begin{align}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \draw[red!80!black, line width=1.2pt, domain=\xmin:\xmax, samples=2]
            plot (\x, {lone(\x)})
            node[pos=0.78, above right] {\(\n{\textcircled{\scriptsize 2}} \ \D \hat k_t=0\)};
...
%         % Nullclines and stable arm
%         \draw[red!80!black, line width=1.2pt, domain=0:\knonxmax, samples=200]
%             plot (\x, {knon(\x)})
